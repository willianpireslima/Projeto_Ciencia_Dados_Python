...
df = pd.read_csv('dados/churn_model.csv')
...
Exited_corr = df.corr()["Exited"]
Exited_corr = Exited_corr.drop("Exited", axis=0).sort_values(ascending=False)

# Criando o gráfico de barras
plt.figure(figsize=(10,6))
sns.barplot(x=Exited_corr.values, y=Exited_corr.index)
...
X = df.drop(['Exited'], axis=1) # Variáveis independentes (features)
y = df['Exited'] # Variável dependente (target)
